...
class ScrapeJobSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ScrapeJob
        fields = [
            "id", "source_name", "status", "region", "countries",
...
class IngestedLayerSerializer(serializers.ModelSerializer):
    class Meta:
        model = IngestedLayer
        fields = [
            "id", "name", "iso3", "admin_level",
            "db_schema", "db_table", "geom_type", "srid",
            "feature_count", "properties",
            "source_url", "source_format", "created_at",
        ]
