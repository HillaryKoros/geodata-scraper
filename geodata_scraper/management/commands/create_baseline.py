...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--output-schema", type=str, default="geodata",
    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cur:
            ...
            if not available:
                self.stderr.write("  ERROR: No admin0 tables found. Run GADM scrape first.")
                return

            self.stdout.write(f"  Countries found: {', '.join(c.upper() for c in available)} ({len(available)}/11)")
            ...
            cur.execute(f'DROP TABLE IF EXISTS "{out_schema}"."igad_admin0_merged" CASCADE')
            cur.execute(f'''
            ...
            cur.execute(f'DROP TABLE IF EXISTS "{out_schema}"."igad_baseline_simplified" CASCADE')
            cur.execute(f'''
            ...
            self.stdout.write(f"    Simplified: {simp_pts:,} vertices ({reduction:.0f}% reduction)")
            self.stdout.write(f"    Total area: {area_km2:,.0f} km2")

            # Step 4: Spatial indexes
            self.stdout.write("  [4/4] Creating spatial indexes...")
            for tbl in ["igad_admin0_merged", "igad_baseline", "igad_baseline_simplified"]:
                cur.execute(f'''
        ...
        self.stdout.write("=" * 60)
        self.stdout.write(f"  {out_schema}.igad_admin0_merged      — {count} country polygons")
        self.stdout.write(f"  {out_schema}.igad_baseline            — 1 dissolved IGAD+ outline")
        self.stdout.write(f"  {out_schema}.igad_baseline_simplified — 1 simplified outline")
