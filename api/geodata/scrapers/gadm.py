...
log = logging.getLogger("geodata.gadm")
...
class GADMScraper(BaseScraper):
    ...
    def build_tasks(self, countries: list[str], **kwargs) -> list[dict]:
        storage_dir = kwargs.get("storage_dir", Path("/tmp/geodata"))
        tasks = []
            ...
            if self.format == "geojson":
                ...
                for level in range(max_level + 1):
                    ...
                    dest = (
                        ...
                        / "gadm"
                        / iso3
                        / f"{GADM_PREFIX}_{iso3}_{level}.json"
                    )
                    tasks.append(
                        {
                            "url": url,
        ...
        for attempt in range(1, retries + 1):
            try:
                with httpx.stream(
                    ...
                    _total = int(resp.headers.get("content-length", 0))  # noqa: F841
                    ...
                    tmp.rename(dest)
                    level_str = (
                        f"admin{admin_level}" if admin_level >= 0 else "all-levels"
                    )
                    log.info(
                        f"OK: {iso3} {level_str} — {dest.name} ({_fmt_size(downloaded)})"
                    )
    ...
    try:
        ...
        for line in result.stdout.strip().splitlines():
            ...
            if len(parts) >= 2:
                ...
                geom = (
                    parts[1].strip().split("(")[1].rstrip(")").strip()
                    if "(" in parts[1]
                    else ""
                )
                if name:
